...
function updateFramePreview() { const [name, color, fill, size] = framePresets[selectedFrameIndex]; const preview = $('#frameLivePreview'); preview.style.setProperty('--frame-color', color); preview.style.setProperty('--frame-fill', fill); preview.style.setProperty('--preview-border', `${Math.max(34, Math.round(size * 1.65))}px`); $('#framePreviewTitle').textContent = name; }
function renderFrameOptions() { $('#frameGrid').innerHTML = framePresets.map(([name, color, fill, size], index) => `<button class="frame-option ${index === selectedFrameIndex ? 'active' : ''}" data-frame-index="${index}" type="button"><span class="frame-swatch" style="--frame-color:${color};--frame-fill:${fill};--frame-size:${Math.max(11, Math.round(size / 2.5))}px"></span>${name}</button>`).join(''); updateFramePreview(); }
...
async function addFrameToImage(url, frame) { const image = await loadExportImage(url); const [name, color, fill, basePadding] = frame; const padding = Math.max(basePadding * 2, Math.round(Math.min(image.naturalWidth, image.naturalHeight) * .115)); const canvas = document.createElement('canvas'); canvas.width = image.naturalWidth + padding * 2; canvas.height = image.naturalHeight + padding * 2; const context = canvas.getContext('2d'); context.fillStyle = fill; context.fillRect(0, 0, canvas.width, canvas.height); context.strokeStyle = color; context.lineWidth = Math.max(12, Math.round(padding * .16)); context.strokeRect(Math.round(context.lineWidth / 2), Math.round(context.lineWidth / 2), canvas.width - context.lineWidth, canvas.height - context.lineWidth); context.drawImage(image, padding, padding, image.naturalWidth, image.naturalHeight); const signatureSize = Math.max(18, Math.round(padding * .28)); context.save(); context.translate(canvas.width - Math.round(padding * .5), canvas.height - Math.round(padding * .42)); context.rotate(-0.05); context.fillStyle = color; context.font = `italic 700 ${signatureSize}px cursive`; context.textAlign = 'right'; context.textBaseline = 'alphabetic'; context.fillText('Prompt Pop!', 0, 0); context.font = `600 ${Math.max(10, Math.round(signatureSize * .48))}px sans-serif`; context.fillText(name.toUpperCase(), 0, Math.round(signatureSize * .68)); context.restore(); return canvas.toDataURL('image/png'); }
