...
async function addFrameToImage(url, frame) { const image = await loadExportImage(url); const [name, color, fill, padding] = frame; const canvas = document.createElement('canvas'); canvas.width = image.naturalWidth + padding * 2; canvas.height = image.naturalHeight + padding * 2; const context = canvas.getContext('2d'); context.fillStyle = color; context.fillRect(0, 0, canvas.width, canvas.height); context.fillStyle = fill; context.fillRect(Math.round(padding * .35), Math.round(padding * .35), canvas.width - Math.round(padding * .7), canvas.height - Math.round(padding * .7)); context.drawImage(image, padding, padding, image.naturalWidth, image.naturalHeight); const signatureSize = Math.max(12, Math.round(padding * .42)); context.save(); context.translate(canvas.width - Math.round(padding * .32), canvas.height - Math.round(padding * .3)); context.rotate(-0.05); context.fillStyle = color === '#111' ? '#fff' : color; context.font = `italic 700 ${signatureSize}px cursive`; context.textAlign = 'right'; context.textBaseline = 'alphabetic'; context.fillText('Prompt Pop!', 0, 0); context.font = `600 ${Math.max(8, Math.round(signatureSize * .5))}px sans-serif`; context.fillText(name.toUpperCase(), 0, Math.round(signatureSize * .62)); context.restore(); return canvas.toDataURL('image/png'); }
