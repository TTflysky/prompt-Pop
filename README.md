...
面向手机、Windows 和浏览器的 AI 提示词与出图工作台。它把文本提示词优化、图像风格拆解、文生图、图生图、图片预设和本地配置备份集中在一个应用里。 生成图片时，文生图和图生图都会显示小花生长进度动画，完成后花朵盛开。
...
- 当前便携版：`release/Prompt-Pop-Windows-v1.2.48.zip`
- 解压后直接运行 `Prompt Pop 1.2.48.exe`，无需安装。
...
2. 可选“使用通用提示词控件”。修改风格、视角、光线、构图、焦段或滑块时，只会追加或更新该项中文内容，不会清空手写提示词；输出画质和画布尺寸只影响接口参数。
3. “优化生图词”只改写当前提示词，不会开始生成。
4. 分别选择画质（1K、2K、4K）和画面比例，再点击“文生图”。`gpt-image-2` 的 4K 横图会按标准 UHD `3840 × 2160` 请求；如果中转站降采样，界面会同时显示请求像素和实际像素，保留服务端原图而不做 APP 放大。
...
在 Android 设置页点击“检查更新”。发现版本后选择“立即更新”，应用会下载最新网页资源并重新加载。当前版本为 `v1.2.48`。
